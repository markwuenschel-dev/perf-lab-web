    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-8 backdrop-blur-[4px]" style={{ background: "rgba(4,5,8,.68)" }}>
      <div className="max-h-[92vh] w-[780px] max-w-full overflow-auto rounded-[18px] border border-white/[0.09] bg-surface shadow-[0_50px_110px_-30px_rgba(0,0,0,.75)]">
              ...
              <div className="flex flex-col gap-[10px]">
                {scaled.map((v, i) => (
                  <div key={DOSE_NAMES[i]} className="flex items-center gap-[9px]">
                    ...
                    <div className="h-[6px] flex-1 overflow-hidden rounded-full bg-white/[0.07]"><div className="h-full rounded-full" style={{ width: `${v * 10}%`, background: doseBarColor(v) }} /></div>
        ...
        <div className="flex items-center justify-between gap-[9px] border-t border-white/[0.06] px-6 py-4">
          <span className="max-w-[300px] text-[11px] font-medium leading-[1.4] text-dim">Applying pushes D(t) into the twin, advancing fatigue, tissue &amp; capacity.</span>
          <div className="flex flex-none gap-[9px]">
            ...
            <button onClick={actions.applyLog} className="rounded-[9px] bg-gradient-to-r from-ac to-[#a7e36e] px-[18px] py-[11px] text-[12.5px] font-semibold leading-none text-[#0a0c10]">Apply to twin →</button>
          </div>
        </div>
      </div>
    </div>
